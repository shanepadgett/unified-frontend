...
import {
  getFeatureFlagDirect,
  updateFeatureFlagDirect,
  deleteFeatureFlagDirect
...
function EditFeatureFlagPage() {
  ...
  const { data: flag, isLoading, isError } = useQuery<FeatureFlag>({
    ...
    queryFn: async () => {
      console.log('Fetching feature flag with ID:', id);
      try {
        // Use the direct API function instead of the server function
        const result = await getFeatureFlagDirect(id);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: UpdateFeatureFlag) => {
      console.log('Updating feature flag with ID:', id, 'and data:', data);
      try {
        // Use the direct API function instead of the server function
        return await updateFeatureFlagDirect(id, data);
      } catch (error) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        // Use the direct API function instead of the server function
        return await deleteFeatureFlagDirect(id);
      } catch (error) {
